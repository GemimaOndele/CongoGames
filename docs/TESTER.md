...
- **Fond 3D (plateau TV)** : sans vidéo dans `StreamingAssets/Theme/`, le fond peut être un **rendu URP** (`VirtualShowStage3D` → RenderTexture). Désactiver : **F9** → décocher « Fond 3D… » ou `PlayerPrefs` **`CongoUseVirtual3D=0`** puis relancer Play. Avec une **vidéo** de fond, la vidéo reste prioritaire.
