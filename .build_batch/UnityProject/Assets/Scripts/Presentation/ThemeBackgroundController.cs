...
namespace CongoGames.Presentation
{
    ...
    public class ThemeBackgroundController : MonoBehaviour
    {
        ...
        [SerializeField] private float videoCycleSeconds = 7f;
        ...
        private Coroutine applyCo;
        [SerializeField] private float minVideoSwitchSeconds = 5f;
        ...
        private readonly HashSet<string> failedVideoUrls = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        private bool videoErroredThisClip;
        ...
        public void ApplyGameMode(string modeId)
        {
            ...
            if (applyCo != null)
            {
                StopCoroutine(applyCo);
            }

            applyCo = StartCoroutine(CoApplyGameMode(id));
        }
        ...
        private IEnumerator CoApplyGameMode(string modeId)
        {
            ...
            validCandidates = validCandidates
                .Select(StreamingAssetsUrl.ToRequestUrl)
            ...
            // En build Windows on veut d'abord voir les MP4 Theme défiler clairement.
            // Le 3D reste disponible uniquement quand aucun fichier vidéo n'est trouvé.
            bool mix3DWithVideo = false;
        ...
        private IEnumerator CoGatherVideoCandidates(string id, List<string> validCandidates)
        {
            ...
            ThemeModeCatalog.AppendVideosFromOtherModeFolders(id, validCandidates);
            if (StreamingAssetsUrl.IsWebGlData)
            {
                foreach (string u in ThemeModeCatalog.EnumerateStandardVideoUrlsForOtherModes(id))
                {
                    bool ok = false;
                    yield return WebGlStreamingPrewarm.CoHttpHeadOk(u, b => ok = b);
                    if (ok)
        ...
        {
            ReleaseVideoPlayerAndRenderTexture(true);
        }

        private void ReleaseVideoPlayerAndRenderTexture(bool clearDebug)
        {
            if (video != null)
            {
        ...
        {
            ReleaseVideoPlayerAndRenderTexture(false);
            videoErroredThisClip = false;
            currentVideoUrl = url ?? "";
        ...
        private IEnumerator CoCycleVideos(string modeId, List<string> urls)
        {
            ...
            while (!string.IsNullOrEmpty(activeModeId) && string.Equals(activeModeId, modeId, StringComparison.Ordinal))
            {
                ...
                {
                    int candidateIdx = (idx + attempt) % urls.Count;
                    string candidateUrl = urls[candidateIdx];
                    if (failedVideoUrls.Count < urls.Count && failedVideoUrls.Contains(candidateUrl))
                    {
                        continue;
                    }
        ...
        {
            string badUrl = !string.IsNullOrEmpty(source != null ? source.url : null)
                ? source.url
                : currentVideoUrl;
            if (!string.IsNullOrEmpty(badUrl))
            {
                failedVideoUrls.Add(badUrl);
            }

            if (!string.IsNullOrEmpty(message))
            {
                Debug.LogWarning("ThemeBackground vidéo ignorée: " + message + " (" + debugVideoUrl + ")");
            }

            videoErroredThisClip = true;
            ReleaseVideoPlayerAndRenderTexture(false);

            if (videoCycleCo == null && alternateMixCo == null)
            {
